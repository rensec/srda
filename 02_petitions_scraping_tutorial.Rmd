...
We can now do the same for our next variable, the URL. Looking at the result of our first processing step for the titles (the one resulting from `html_nodes(...)` above), we see that the URLs are already included there (starting after "href" in each line); we just need to extract them. 

The code to get all the URLs then looks like this:

```{r}
link_url <- html_nodes(webpage, ".petition-overview-info-title") # This leads to same result as above 
link_url <- html_nodes(link_url, "a") # we first keep all the parts that start with "a"
link_url <- html_attr(link_url, "href") # then, we extract the links and show the result 
link_url
