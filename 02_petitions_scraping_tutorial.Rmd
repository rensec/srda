...
install.packages(
  c(
    ...
    "stringr", # some useful fucntions for working with strings
...
```{r}
link_url <- webpage %>%
    html_elements(".petition-overview-image-container")%>% 
    html_attr("href") 
  
```

Note that now we use the "pipe operator" `%>%` to simplify the code somewhat (but we could also have done it without as above).
...
get_petitions_list <- function(page_url){
  ...
  title <- webpage %>%
    html_nodes(".petition-overview-info-title") %>% 
    html_text() 
  ...
  sig_count <- webpage %>% 
    html_elements(".petitions-counter") %>% 
    html_text() 
...
get_petitions_list <- function(page_url){
  ...
  sig_count <- webpage %>% 
    html_elements(".petitions-counter") %>% 
    html_text() %>% 
    str_replace_all("\\.","") %>% # NEW: remove Dutch 1000 separator
    as.numeric() # NEW: change the type from string to numeric
...
for(i in 2:5){ # Loop throup all values from 2 to 5. The current value is "i" 
    print(paste("scraping page",i,sep=)) #NEW: print what's happening to the console
